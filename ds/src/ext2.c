...
static void read_super_block(FS_t *fs)
{
    lseek(fs->fd, 1024, SEEK_SET);  /*Moves the file pointer to the location of the superblock (offset 1024 bytes)*/ 
    if (read(fs->fd, &fs->sb, sizeof(struct ext2_super_block)) != sizeof(struct ext2_super_block))  /*Checks that the size of the filesystem's superblock is the same size as our super_block struct and loads it into our struct.*/
    {
    ...
    fs->block_size = 1024 << fs->sb.s_log_block_size;  /*Example: if s_log_block_size is 2, then 1024 << 2 = 1024 * 2^2 = 4096 (4 kb block size).*/
    printf("Block size: %u\n", fs->block_size);
}
...
{
    uint32_t num_groups;
    uint32_t gd_size;  /*Size of group descriptor array*/
...
FS_t* FSOpen(const char* disk_name)
{
    ...
    fs->fd = open(disk_name, O_RDONLY);  /*Opens the disk specified by disk_name (/dev/ram0 in my case) in read-only mode (O_RDONLY), and returns a file descriptor which is stored in fs->fd.*/
    if (fs->fd == -1) 
    {
...
static uint32_t search_dir(FS_t *fs, uint32_t inode_num, const char *name)
{
    struct ext2_inode *inode;  /*Pointer to an inode structure for the specified directory.*/
    uint8_t *block;  /*Pointer to memory allocated for a block of directory entries.*/
    struct ext2_dir_entry *entry;  /*Pointer to a directory entry structure.*/
    uint32_t offset;  /*Keeps track of the current position in the directory block.*/
    ...
    if (read(fs->fd, block, fs->block_size) != fs->block_size)  /*Reads the contents of the directory block into "block".*/
    {
...
long GetInode(FS_t* file_system, const char* file_path)
{
    ...
    const char *relative_path;  /*The portion of file_path after the mount point.*/
    char *path;  /*A copy of relative_path that will be tokenized (split) to traverse the directories.*/
    char *token;  /*Each directory or file component (separated by /) from the path.*/
    uint32_t inode_num;  /*The inode number of the current directory or file being searched.*/
...
void PrintFile(FS_t* file_system, long inode_num)
{
    ...
    for (i = 0; i < 12 && inode->i_block[i]; ++i)  /*The loop iterates up to 12 times (the maximum number of direct blocks for an inode in ext2) as long as there are valid block pointers in inode->i_block[i].*/
    {
        ...
        bytes_to_print = (i == 11 || !inode->i_block[i+1]) 
